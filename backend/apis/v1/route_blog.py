...
def create_blog(blog: CreateBlog, db: Session = Depends(get_db), current_user: User = Depends(get_current_active_user)):
    blog = create_new_blog(db=db, blog=blog, author_id=current_user.id)
    return blog

@router.get("/{id}", response_model=ShowBlog)
def get_blog(id: int, db: Session = Depends(get_db)):
    # blog = db.query(Blog).filter(Blog.id == id).first()
    blog = retrieve_blog(db=db, id=id)
    if not blog:
...
def update_blog(id: int, blog: UpdateBlog, db: Session = Depends(get_db), current_user: User = Depends(get_current_active_user)):
    blog = update_blog_by_id(db=db, id=id, blog=blog, author_id=current_user.id)
...
@router.delete("/{id}")
def delete_a_blog(id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_active_user)):
